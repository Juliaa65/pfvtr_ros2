# jackal_backwards.launch.py  --  bidirectional mapping for this Jackal
#
# Concrete topic names for this machine, modeled on dual_map.launch.py.
# Starts TWO pfvtr pipelines from a single launch:
#
#   /pfvtr      - full teach+repeat stack on the front camera (camera1).
#   /pfvtr_map  - mapping-only stack on the rear camera (camera2), used to
#                 record a second map while /pfvtr teaches/repeats the front.
#
# Trigger the rear recording with, e.g.:
...
#
# See dual_map.launch.py for the full rationale (why two representations
# nodes are needed, and how the post-processed backward map is repeated).
...
def generate_launch_description():
    ...
    camera_topic = DeclareLaunchArgument(
        ...
        description="Front RealSense camera topic",
    )
    ...
    camera_back_topic = DeclareLaunchArgument(
        ...
        description=(
            "Rear RealSense camera topic, rebound in by the /pfvtr_map "
            "mapmaker when its MapMaker action is called with "
