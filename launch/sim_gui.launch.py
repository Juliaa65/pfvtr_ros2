...
def generate_launch_description():
    ...
    kde_max_step_back = DeclareLaunchArgument(
        ...
        default_value="0.1",
        description=(
            "Max backward Δdistance (m) per visual KDE update. "
            "<=0 disables. Odometry uncapped."
        ),
    )
    kde_max_step_fwd = DeclareLaunchArgument(
        "kde_max_step_fwd",
        default_value="0.1",
        description=(
            "Max forward Δdistance (m) per visual KDE update. "
            "<=0 disables. Odometry uncapped."
        ),
    )
