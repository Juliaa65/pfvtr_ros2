    ...
    def forward(self, x):
        if self.legacy:
            ...
            if self.res > 0:
                ...
                x = self.l1(x); x = t.relu(x)
                x = self.l2(x); x = t.relu(x)
                x = self.l3(x); x = t.relu(x)
                x = self.l4(x); x = t.relu(x)
                x = self.l5(x)
            return x
...
class Siamese(t.nn.Module):
    ...
    def match_corr(self, embed_ref, embed_srch, padding=None):
        ...
        if self.training:
            ...
            match_map = F.conv2d(
                F.pad(embed_srch.view(1, b * c, h, w), pad=(padding, padding, 1, 1), mode="circular"),
                embed_ref,
                groups=b,
            )
...
def load_model(model, path, optimizer=None, logger=None):
    log = logger if logger is not None else get_logger("siam_model")

    checkpoint = t.load(path, map_location=t.device("cpu"), weights_only=False)
    model.load_state_dict(checkpoint['model_state_dict'])
    log.info(f"Loaded model at {path}")
