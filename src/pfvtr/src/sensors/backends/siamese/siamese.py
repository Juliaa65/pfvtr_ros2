...
from pfvtr.msg import SensorsInput, Features
...
class SiameseCNN(DisplacementEstimator, ProbabilityDistanceEstimator,
    ...
    def __init__(self, padding: int = 32, resize_w: int = 512, path_to_model=None, logger=None):
        super(SiameseCNN, self).__init__()
        self.supported_message_type = SensorsInput

        self.log = logger if logger is not None else get_logger("siamese_cnn")
    ...
    def _from_feature(self, msg: Features):
        return t.stack(
            [t.tensor(np.array(feature.values).reshape(feature.shape)) for feature in msg],
            dim=0
        ).to(self.device)
    ...
    def process_msg(self, msg):
        ...
        if interpolate:
            ...
            for hist in interp_hist:
                ...
                ret.append(np.concatenate([zeros, hist, zeros]))
            self.histograms = ret
        else:
