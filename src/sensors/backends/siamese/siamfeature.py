...
from pfvtr.msg import SensorsInput, Features, Descriptor
...
class SiamFeature(DisplacementEstimator, ProbabilityDistanceEstimator,
    ...
    def __init__(self, padding: int = 32, resize_w: int = 512, descriptor: str = "BRISK", path_to_model=None, logger=None):
        super(SiamFeature, self).__init__()
        self.supported_message_type = SensorsInput

        self.log = logger if logger is not None else get_logger("siam_feature")
        self._bridge = CvBridge()

        self.descriptor = descriptor
        self.device = t.device("cuda") if t.cuda.is_available() else t.device("cpu")
        self.log.warning("Neural net device " + str(self.device))
        ...
        self.model = load_model(model, path_to_model, logger=self.log).to(self.device).float()
        ...
        self.log.info("Siamese-NN displacement/distance estimator successfully initialized!")
    ...
    def _abs_dist_message_callback(self, msg: SensorsInput) -> float:
        ...
        if not len(msg.distances) > 0:
            self.log.error("You cannot assign absolute distance to ")
    ...
    def _from_feature(self, msg: Features):
        a = t.stack([t.tensor(np.array(feature.values).reshape(feature.shape)) for feature in msg], dim=0).to(self.device)
    ...
    def _to_feature(self, msg: Image) -> Features:
        with t.no_grad():
            ...
            for repr in reprs:
                f = Features()
                f.shape = t.tensor(repr.shape).cpu().numpy()
    ...
    def get_descriptors(self, msg: Image):
        ...
        if self.descriptor not in self.featureTypes:
            self.log.warning("Feature type unknown/unavailable. Not correcting heading!")
            return descriptors

        for i in msg:
            kps, des = self.featureTypes[self.descriptor].detectAndCompute(self._bridge.imgmsg_to_cv2(i), None)
            if des is not None:
                ...
                self.log.warning("No descriptors found!")
    ...
    def process_msg(self, msg):
        ...
        ret = []
        for h in interp_hist:
            zeros = np.zeros(np.size(h) // 2)
            ret.append(np.concatenate([zeros, h, zeros]))  # siam can do only -0.5 to 0.5 img so extend both sides by sth
        self.histograms = ret
        return ret
    ...
    def matching(self, map, querry, histograms):  ## recoded function from C++ to python
        ...
        for idx, m in enumerate(map):
            q = querry[0] if idx >= len(querry) else querry[idx]
    ...
    def image_to_tensor(self, imgs):
        ...
        for img in imgs:
            ...
            image_list.append(
                transforms.Resize(desired_height, interpolation=InterpolationMode.NEAREST)(
